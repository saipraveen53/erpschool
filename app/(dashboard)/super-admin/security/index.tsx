...
const SECURITY_BASE_URL = "http://192.168.88.20:8083";
...
export default function SecurityManagement() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await rootApi.get(
        `${SECURITY_BASE_URL}/api/superAdmin/users`,
      );
      if (response.data) {
  ...
  const toggleUserStatus = async (
    ...
    try {
      await rootApi.put(
        `${SECURITY_BASE_URL}/api/superAdmin/users/${username}/status?active=${newStatus}`,
      );
      setUsers((prev) =>
        prev.map((u) =>
          u.username === username ? { ...u, isAvailable: newStatus } : u,
        ),
      );
    } catch (error: any) {
      ...
      >
        ...
        >
          <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
            ...
            <Text
              style={[styles.headerTitle, { fontSize: isMobile ? 20 : 26 }]}
            >
              Security & Access
            </Text>
          </View>
          <Text style={styles.headerSubtitle}>
            Manage users, their roles, and what they can access in the system.
          </Text>
        </View>

        <View
          style={[styles.searchContainer, { width: isMobile ? "100%" : 300 }]}
        >
            ...
            {filteredUsers.map((user, idx) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    {isMobile && (
                      <Text style={styles.mobileLabel}>Access Toggle:</Text>
                    )}
                    {user.role === "SUPER_ADMIN" ? (
                      ...
                      <Switch
                        value={isActive}
                        onValueChange={() =>
                          toggleUserStatus(user.username, isActive, user.role)
                        }
