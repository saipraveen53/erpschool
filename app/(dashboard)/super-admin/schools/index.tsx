    ...
    <View style={styles.container}>
      ...
      <View style={{ marginHorizontal: isMobile ? 12 : 20, marginTop: 20 }}>
        <View style={styles.searchBox}>
          ...
          <TextInput style={styles.searchInput} placeholder="Search schools by name or domain..." placeholderTextColor="#B8A095" value={searchQuery} onChangeText={setSearchQuery} />
        </View>
              ...
              <View style={styles.cardMeta}>
                ...
                <View style={styles.metaItem}>
                  ...
                  <TouchableOpacity onPress={() => toggleStatus(school.id)}>
                    <View style={[styles.statusBadge, { backgroundColor: school.status === "Active" ? "#dcfce7" : "#F4A460" }]}>
                      <Text style={[styles.statusText, { color: school.status === "Active" ? "#166534" : "#8A6B5D" }]}>{school.status}</Text>
                    </View>
                  </TouchableOpacity>
                </View>
                <View style={styles.cardActions}>
                  <TouchableOpacity style={styles.actionBtn} onPress={() => openEdit(school)}><Edit size={15} color="#8A6B5D" /></TouchableOpacity>
            ...
            {filteredSchools.map(school => (
              <TouchableOpacity key={school.id} style={styles.tableRow} onPress={() => router.push(`/super-admin/schools/${school.id}` as any)}>
                ...
                <View style={[styles.td, { flex: 2.5, flexDirection: "row", alignItems: "center" }]}>
                  <View style={styles.iconContainer}><Building2 size={15} color="#E35336" /></View>
                  <View style={{ flex: 1 }}>
                ...
                <View style={[styles.td, { flex: 1, flexDirection: "row", justifyContent: "center" }]}>
                  <TouchableOpacity style={styles.actionBtn} onPress={() => openEdit(school)}><Edit size={14} color="#8A6B5D" /></TouchableOpacity>
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { width: isMobile ? "92%" : "90%", maxWidth: 500 }]}>
            <View style={styles.modalHeader}>
              ...
              <TouchableOpacity onPress={() => setModalVisible(false)}><X size={24} color="#8A6B5D" /></TouchableOpacity>
            </View>
...
const styles = StyleSheet.create({
  ...
  cardIconWrap: { width: 36, height: 36, borderRadius: 10, backgroundColor: "#F4A460", justifyContent: "center", alignItems: "center", marginRight: 12 },
  cardName: { fontSize: 15, fontWeight: "700", color: "#A0522D" },
  cardDomain: { fontSize: 13, color: "#8A6B5D", marginTop: 2 },
  cardMeta: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingTop: 10, borderTopWidth: 1, borderTopColor: "#F4A460" },
  metaItem: { alignItems: "flex-start" },
  metaLabel: { fontSize: 11, color: "#B8A095", fontWeight: "500", marginBottom: 4 },
  metaValue: { fontSize: 14, fontWeight: "700", color: "#A0522D" },
  ...
  iconContainer: { width: 32, height: 32, borderRadius: 8, backgroundColor: "#F4A460", justifyContent: "center", alignItems: "center", marginRight: 12, flexShrink: 0 },
  schoolName: { fontSize: 14, fontWeight: "600", color: "#A0522D" },
  schoolDomain: { fontSize: 13, color: "#8A6B5D" },
  planText: { fontSize: 13, color: "#705244", fontWeight: "500" },
  usersText: { fontSize: 13, color: "#705244" },
  ...
  actionBtn: { padding: 6, backgroundColor: "#F4A460", borderRadius: 6 },
  // Modal
