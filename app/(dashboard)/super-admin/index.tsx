...
export default function SuperAdminDashboard() {
  ...
  const fetchBillingStats = async () => {
    try {
      const response = await rootApi.get(
        `http://192.168.88.20:8083/api/student/fee/admin/dashboard/stats`,
      );
      if (response.data && Array.isArray(response.data)) {
        const expected = response.data.reduce(
          (sum, item) => sum + (item.totalExpectedFee || 0),
          0,
        );
        const collected = response.data.reduce(
          (sum, item) => sum + (item.totalCollectedFee || 0),
          0,
        );
        const pending = response.data.reduce(
  ...
  const fetchNotices = async () => {
    try {
      const response = await rootApi.get(
        "http://192.168.88.20:8083/api/student/notice/all",
      );
      if (response.data && Array.isArray(response.data)) {
  ...
  const fetchDashboardMetrics = async () => {
    try {
      const countRes = await rootApi
        .get("/api/student/count")
        .catch((e) => ({ data: 0 }));
      const genderRes = await rootApi
        .get("/api/student/dashboard/gender-percentage")
        .catch((e) => ({ data: null }));
      ...
      const overviewRes = await rootApi
        .get("http://192.168.88.20:8083/api/superAdmin/dashboard")
        .catch(() => ({ data: {} }));
  ...
  const fetchDashboardUsers = async () => {
    try {
      ...
      const response = await rootApi.get(
        `http://192.168.88.20:8083/api/superAdmin/users`,
      );
      if (response.data && Array.isArray(response.data)) {
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            try {
              await rootApi.put(
                `http://192.168.88.20:8083/api/superAdmin/users/${username}/status?active=${newStatus}`,
              );
              setDashboardUsers((prev) =>
                prev.map((u) =>
                  u.username === username
                    ? { ...u, isAvailable: newStatus }
                    : u,
                ),
              );
            } catch (error) {
  ...
  const generateExcel = async () => {
    try {
      ...
      const feesRes = await rootApi
        .get(`http://192.168.88.20:8083/api/student/fee/admin/dashboard/stats`)
        .catch(() => ({ data: [] }));
      const detailedFees = Array.isArray(feesRes.data) ? feesRes.data : [];

      const pendingLeavesRes = await rootApi
        .get(`/api/teacher/leave/byStatus?status=PENDING`)
        .catch(() => ({ data: [] }));
      const approvedLeavesRes = await rootApi
        .get(`/api/teacher/leave/byStatus?status=APPROVED`)
        .catch(() => ({ data: [] }));
      const rejectedLeavesRes = await rootApi
        .get(`/api/teacher/leave/byStatus?status=REJECTED`)
        .catch(() => ({ data: [] }));
      const allLeaves = [
        ...(Array.isArray(pendingLeavesRes.data) ? pendingLeavesRes.data : []),
        ...(Array.isArray(approvedLeavesRes.data)
          ? approvedLeavesRes.data
          : []),
        ...(Array.isArray(rejectedLeavesRes.data)
          ? rejectedLeavesRes.data
          : []),
      ];

      const usersRes = await rootApi
        .get(`http://192.168.88.20:8083/api/superAdmin/users`)
        .catch(() => ({ data: [] }));
  ...
  return (
    <ScrollView
      ...
      contentContainerStyle={{ padding: isMobile ? 12 : 28, paddingBottom: 60 }}
    >
      <View
        style={[
          styles.pageHeader,
      ...
      >
        <View>
          <Text style={[styles.pageTitle, { fontSize: isMobile ? 22 : 30 }]}>
            System Overview
          </Text>
          <Text style={styles.pageSubtitle}>
            Real-time analytics and platform health
          </Text>
        </View>
      ...
      <View
        style={{ flexDirection: "column", width: "100%", flex: 1, gap: 20 }}
      >
          ...
          <View style={[styles.card, { padding: isMobile ? 16 : 24 }]}>
            <View style={styles.cardHeaderFlex}>
              <View>
                ...
                <Text style={styles.cardSubtitle}>
                  School-wide pass rates and active enrollments
                </Text>
              </View>
              <View
                style={[
            ...
            >
              ...
              >
                ...
                >
                  {globalMetrics.totalActiveStudents}
                </Text>
              </View>
              <View
                style={{
              ...
              >
                ...
                >
                  {globalMetrics.totalClassSections}
                </Text>
              </View>
              <View
                style={{
              ...
              >
                ...
                >
                  {globalMetrics.overallPassPercentage}%
                </Text>
              </View>
            </View>
          </View>
              ...
              {(() => {
                ...
                const data = [
                  ...
                  },
                  {
                    ...
                    val: d,
                    col: "#E35336",
                    bg: "rgba(227, 83, 54, 0.15)",
                  },
                  {
                    label: "Total",
                    val: u,
                    col: "#5C2E14",
                    bg: "rgba(92, 46, 20, 0.15)",
                  },
                ];
          ...
          <View style={[styles.card, { flex: 1, padding: isMobile ? 16 : 24 }]}>
            <View style={styles.cardHeaderFlex}>
              <View>
                ...
                <Text style={styles.cardSubtitle}>
                  Compare active and disabled accounts
                </Text>
              </View>
              <View
                style={[
            ...
            <View style={{ flex: 1, justifyContent: "center" }}>
              {loadingUsers ? (
                ...
                <View style={{ alignItems: "center" }}>
                  ...
                  <Text
                    style={{ color: "#8A6B5D", fontSize: 12, marginTop: 10 }}
                  >
                ...
                <View style={{ alignItems: "center" }}>
                  {(() => {
                    const activeCount = dashboardUsers.filter(
                          ...
                          >
                            <G rotation="-90" origin={`100, 70`}>
                              ...
                              <Circle
                                ...
                                strokeWidth="20"
                                fill="transparent"
                                strokeDasharray={`${aDash * (50 / 64)} ${circumference * (50 / 64)}`}
                                strokeLinecap="round"
                              />
                            </G>
                            <SvgText
        ...
        <View style={{ flexDirection: isMobile ? "column" : "row", gap: 16 }}>
          <TouchableOpacity
            ...
            onPress={() => router.push("/super-admin/billing" as any)}
          >
            <View
              style={{
            ...
            <View>
              ...
              <Text
                style={{ fontSize: 20, fontWeight: "900", color: "#1C1917" }}
              >
                ₹{billingStats.expected.toLocaleString("en-IN")}
              </Text>
            </View>
          </TouchableOpacity>
          ...
          >
            ...
            <View>
              ...
              <Text
                style={{ fontSize: 20, fontWeight: "900", color: "#16a34a" }}
              >
                ₹{billingStats.collected.toLocaleString("en-IN")}
              </Text>
            </View>
          </TouchableOpacity>
          ...
          >
            ...
            <View>
              ...
              >
                ₹{billingStats.pending.toLocaleString("en-IN")}
              </Text>
            </View>
          </TouchableOpacity>
        </View>
        ...
        <View style={{ flexDirection: isMobile ? "column" : "row", gap: 16 }}>
          <TouchableOpacity
            ...
            onPress={() => router.push("/super-admin/leaves" as any)}
          >
            <View
              style={{
            ...
            <View>
              ...
              <Text
                style={{ fontSize: 20, fontWeight: "900", color: "#ca8a04" }}
              >
                {leaveStats.totalPendingRequests}
              </Text>
            </View>
          </TouchableOpacity>
          ...
          >
            ...
            <View>
              ...
              <Text
                style={{ fontSize: 20, fontWeight: "900", color: "#2563eb" }}
              >
                {leaveStats.teachersOnLeaveToday}
              </Text>
            </View>
          </TouchableOpacity>
          ...
          >
            ...
            <View>
              ...
              <Text
                style={{ fontSize: 20, fontWeight: "900", color: "#dc2626" }}
              >
                {leaveStats.totalRejectedLeaves}
              </Text>
            </View>
          </TouchableOpacity>
        </View>
        ...
        <View style={{ flexDirection: isMobile ? "column" : "row", gap: 20 }}>
          ...
          <View style={[styles.card, { flex: 1, padding: isMobile ? 16 : 24 }]}>
            ...
            >
              ...
              >
                <Text style={[styles.chartTabText, { color: "#F4A460" }]}>
                  Male: {genderData.malePercentage || 0}%
                </Text>
              </View>
              <View
                style={[
              ...
              >
                <Text style={[styles.chartTabText, { color: "#E35336" }]}>
                  Female: {genderData.femalePercentage || 0}%
                </Text>
              </View>
            </View>
          </View>
          ...
          <View style={[styles.card, { flex: 1, padding: isMobile ? 16 : 24 }]}>
            ...
            <View style={{ flex: 1 }}>
              {recentNotices.length === 0 ? (
                ...
                >
                  <Text style={{ color: "#8A6B5D", fontSize: 12 }}>
                ...
                recentNotices.map((notice, index) => (
                  <View
                    ...
                    style={{
                      marginBottom: 14,
                      borderBottomWidth:
                        index === recentNotices.length - 1 ? 0 : 1,
                      borderBottomColor: "#F5E6DF",
                      paddingBottom:
                        index === recentNotices.length - 1 ? 0 : 14,
                    }}
                  >
                    <View
                      style={{
            ...
            <TouchableOpacity
              ...
              onPress={() => router.push("/super-admin/notices" as any)}
            >
              <Text style={styles.premiumButtonOutlineText}>
                Manage All Notices
              </Text>
            </TouchableOpacity>
          </View>
